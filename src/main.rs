...
fn main() {
    App::build()
        ...
        .insert_resource(LastTailPoition::default())
...
fn food_spawner(
    ...
    let spawn_pos = loop {
        ...
        if !snake_segments.iter().any(|pos| *pos == spawn_pos) {
            break spawn_pos;
        };
    };
...
fn snake_movement(
    ...
    mut last_tail_position: ResMut<LastTailPoition>,
...
fn snake_eating(
    ...
    head_postions: Query<&Position, With<SnakeHead>>,
) {
    for head_pos in head_postions.iter() {
...
fn snake_growth(
    commands: Commands,
    last_tail_position: Res<LastTailPoition>,
    mut segments: ResMut<SnakeSegments>,
...
#[derive(Default)]
struct LastTailPoition(Option<Position>);
